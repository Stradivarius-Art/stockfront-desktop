...
(создаётся командой `--seed-admin` только в пустой БД):
...
4. **Администратор заведён:** после `-- --seed-admin` в таблице пользователей есть запись `admin`
   (`SELECT Username FROM stockfront.Users;`).
5. **Запуск:** `dotnet run --project stockfront.csproj` открывает окно входа; под `admin`/`admin123`
   открывается дашборд с карточками-метриками.
