...
Разделы «Витрина» и «Заказы» пока заглушки.
...
# 5. (необязательно) Завести кладовщика по умолчанию
dotnet run --project stockfront.csproj -- --seed-keeper

# 6. (необязательно) Наполнить демо-данными
...
Вход по умолчанию — `admin@stockfront.local` / `admin123` (см. раздел «Вход»).
...
dotnet run --project stockfront.csproj -- --seed-keeper  # кладовщик по умолчанию (можно добавить к существующему админу)

# то же одной командой (порядок: схема → админ → кладовщик):
dotnet run --project stockfront.csproj -- --migrate --seed-admin --seed-keeper
```

Команда `--seed-keeper` идемпотентна: создаёт аккаунт кладовщика, только если его ещё нет (в отличие
от `--seed-admin`, не требует пустой таблицы пользователей — кладовщика можно завести рядом с админом).
...
После запуска появится окно входа. Вход выполняется по **e-mail и паролю**. Учётные записи по
умолчанию (`--seed-admin` создаёт админа только в пустой БД; `--seed-keeper` добавляет кладовщика):

| Роль       | E-mail                    | Пароль      | Команда         |
|------------|---------------------------|-------------|-----------------|
| Админ      | `admin@stockfront.local`  | `admin123`  | `--seed-admin`  |
| Кладовщик  | `keeper@stockfront.local` | `keeper123` | `--seed-keeper` |

Смените пароли после первого входа. Новых пользователей (роль «Покупатель») можно завести через
экран регистрации.
